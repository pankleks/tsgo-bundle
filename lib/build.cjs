...
function build(config, options) {
    ...
    for (const project of selectedProjects(config.projects, options.only)) {
        ...
        // Snapshot only successful builds: a failed build must retry (and fail
        // again) instead of being skipped as up to date.
...
async function watch(config, options) {
    // Native recursive subscriptions (one handle per root); new
    // subdirectories are picked up automatically. Ignore build output,
    // dependencies and dotfiles; only .ts/.json sources schedule rebuilds.
    ...
    function isSourceEvent(event) {
        return (event.path.endsWith(".ts") && !event.path.endsWith(".d.ts")) || event.path.endsWith(".json");
    }
    for (const project of selectedProjects(config.projects, options.only))
        // Test files may live in a second tree (rootDir), watch both.
        for (const directory of [project.projectDir, project.rootDir]) {
            const root = path.join(options.root || process.cwd(), directory);
            if (watched.has(root))
                continue;
            watched.add(root);
            await subscribe(root, (error, events) => {
                if (error != null) {
                    console.error(`[${project.name}] watcher error: ${error.message || error}`);
                    return;
                }
            ...
            }, { ignore: [path.join(root, "node_modules"), path.join(root, ".build"), "**/.*"] });
        }
