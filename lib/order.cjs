const
    ...
    { fs, path, relativeTo, stripComments, isInside } = require("./util.cjs");
...
function orderedSources(project, compiler, root, log) {
    ...
    const
        sources = result.stdout.split(/\r?\n/).map(line => line.trim()).filter(line => line.endsWith(".ts") && !line.endsWith(".d.ts") && isInside(root, line)).map(file => path.resolve(file)),
