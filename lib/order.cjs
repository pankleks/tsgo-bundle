const
    ...
    { fs, path, relativeTo, stripComments, insidePath } = require("./util.cjs");
...
function orderedSources(project, compiler, root, log) {
    ...
    const
        sources = result.stdout.split(/\r?\n/).map(line => line.trim()).filter(line => line.endsWith(".ts") && !line.endsWith(".d.ts")).map(file => insidePath(root, file)).filter(file => file != null),
